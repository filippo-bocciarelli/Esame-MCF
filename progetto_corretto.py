	...
	for l in range (0, len(passi)):
		for k in range (0,len(px)):
			...
			#------------------
				...
				ax[0,0].plot(xdata_hist, binomial(xdata_hist,parx_binom[0], parx_binom[1], parx_binom[2]), c='royalblue',label=r'Binomiale, $ \~{\chi} ^2$='f'{Chi2binom_x[j+l+k]} ')
				ax[0,0].plot(xdata_hist, gaussian_1D(xdata_hist, parx[0],parx[1],parx[2]),label=r'Gaussiana, $ \~{\chi} ^2$='f'{Chi2gauss_x[j+l+k]}', c='darkgreen')
				...
				ax[0,0].set_xlabel('# di passi a dx')
				...
				ax[0,1].set_xlabel('# di passi a dx')
